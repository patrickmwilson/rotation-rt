...
faces = ['face23.jpg', 'face24.jpg', 'face25.jpg']

face1 = (os.path.join(os.getcwd(), 'Stimuli', 'Faces', 'Static', faces[0])) 
face2 = (os.path.join(os.getcwd(), 'Stimuli', 'Faces', 'Static', faces[1]))
face3 = (os.path.join(os.getcwd(), 'Stimuli', 'Faces', 'Static', faces[2])) 

target = face1
distractors = [face2, face3]
...
displayImage = visual.ImageStim( 
    win=win, units='cm', image=face1, 
    size = (trainingWidth,trainingHeight),
    ori = referenceRotation,
    interpolate=True) 
...
for i in range(practiceTrials):
    ...
    if showTarget:
        ...
        face = random.choice(distractors)
